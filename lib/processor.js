...
exports = module.exports = function Processor(command) {
  ...
  this.writeToStream = function(stream, callback) {
	...
	this.transpose = function(config, callback) {
		var self = this;

		var input = (this.options.inputfile ? this.options.inputfile : '-');
		var output = config.outputfile;

		// build transpose command
		var tnArgs = [
			'-i', input,
			'-vf', 'transpose=' + config.transpose
		];
		...
		console.log('Calling ffmpeg with:', tnArgs.join(' '));
		...
		var ffmpegProc = self._spawnProcess(tnArgs);
			...
			ffmpegProc.on('exit', function(code, signal) {
				...
				if (code !== 0) callback(new Error('Unknown error'));
				else callback();
			});
	};
	...
	this.takeScreenshot = function(config, callback) {
		...
		var input = (this.options.inputfile ? self.escapedPath(this.options.inputfile) : '-');
		var output = config.outputfile;
