...
exports = module.exports = function Processor(command) {
    ...
    this._prepare(function(err, meta) {
      if (err) {
        ...
        if (args.length === undefined) {
          ...
          // Modify args before start
          if (self.options.beforeStart) {
              args = self.options.beforeStart(args);
          }
  ...
  this.takeScreenshot = function(config, callback) {
	...
	var input = (this.options.inputfile ? Meta.escapedPath(this.options.inputfile) : '-');
    var output = config.outputfile;//Meta.escapedPath(config.outputfile);
	...
	if (self.options.video.size) {
		tnArgs.push('-s');
		tnArgs.push(self.options.video.size);
	}

	tnArgs.push('-y');
	tnArgs.push(output);

	      var ffmpegProc = self._spawnProcess(tnArgs);
		  // NOTE: I couldn't get this to work reliably with my setup, but the problem may have been somewhere else
          if (self.options.inputstream) {
              // pump input stream to stdin
			  self.options.inputstream.resume();
              self.options.inputstream.pipe(ffmpegProc.stdin);
  ...
  this.buildFfmpegArgs = function(overrideOutputCheck, meta) {
    ...
    // add startoffset (mode: input/output)
    if (this.options.starttime && this.options.starttime_mode != 'output') {
